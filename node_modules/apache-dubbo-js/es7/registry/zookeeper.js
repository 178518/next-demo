...
class ZkRegistry extends registry_1.default {
    constructor(props) {
        ...
        this._init = async (err) => {
            ...
            for (let inf of interfaces) {
                ...
                //当前接口路径下的dubbo url
                const { res: dubboServiceUrls, err } = await go_1.go(this._getDubboServiceUrls(dubboServicePath, inf));
    ...
    _watchWrap(dubboServicePath, dubboInterface) {
        return async (e) => {
            ...
            // await delay(2000);
            const { res: dubboServiceUrls, err } = await go_1.go(this._getDubboServiceUrls(dubboServicePath, dubboInterface));
